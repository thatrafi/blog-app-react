...
export const getAuthentication = (userData) =>{
    return async (dispatch) => {
        ...
        const sendRequest = async () =>{
            const response = await fetch('https://blog-app-react-fb813-default-rtdb.asia-southeast1.firebasedatabase.app/admin.json',
            {
                method : 'GET',
            }
            );
        ...
        try {
           ...
           if(adminData){
               if(adminData.password === userData.password){
                   dispatch(authAction.authenticate({message:'Success', isAuthenticated : true, token : adminData.token}))
               }else{
    ...
    return async (dispatch) => {
        ...
        try {
           ...
           if(adminData){
                ...
                dispatch(authAction.authenticate({message:'Token is not valid', isAuthenticated : false}))
           }
        } catch (error) {
            console.log(error.message);
        }
